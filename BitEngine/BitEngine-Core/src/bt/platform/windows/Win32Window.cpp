...
namespace bt {
	...
	bool Window::PlatformInit() {
		...
		winClass.lpszClassName = "BitEgnine Win32 Window";
		winClass.hCursor = LoadCursor(NULL, IDC_ARROW);
		winClass.hIcon = LoadIcon(NULL, IDI_WINLOGO);

		if (!RegisterClassA(&winClass)) {
			//TODO: handle error
			BT_ERROR("Could not register Win32 class!");
			return false;
		}
		...
		if (pixelFormat) {
			if (!SetPixelFormat(hDc, pixelFormat, &pfd)) {
				BT_ERROR("Failed setting pixel format!");
				return false;
			}
		}
		else {
			BT_ERROR("Failed choosing pixel format!");
			return false;
		}
	...
	void Window::SetTitle(const String& title) {
		m_Title = title + " | " + Application::GetAplication().GetBuildConfiguration() + " " + Application::GetAplication().GetPlatform() + " | Renderer: " + Renderer::GetTitle();

...
namespace bt {
	...
	String StringRange(const String& string, uint start, uint lenght) {
		return string.substr(start, lenght);
	}

	String RemoveStringRange(const String& string, uint start, uint lenght) {
		String result = string;
		return result.erase(start, lenght);
	}
	...
	String GetBlock(const char* str, const char** outPosition = nullptr) {
		...
		if (outPosition)
			*outPosition = end;
		uint lenght = end - str + 1;
		return String(str, lenght);
	}
	...
	String GetStatement(const char* str, const char** outPoaition = nullptr) {
		const char* end = strstr(str, ";");
		if (!end)
			return String(str);
	...
	bool StringWith(const String& string, const String& start) {
